...
if(!function_exists('etheme_wp_title')) {
    function etheme_wp_title($title, $sep ) {
        ...
        if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
            $title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'Shop Jo' ), max( $paged, $page ) );
        }
...
if(!function_exists('etheme_get_the_title')) {
    function etheme_get_the_title() {
        ...
        if( is_404() ) {
            return esc_html__('Page not found', 'Shop Jo');
        }

        if(is_home()) {
            if( empty( $post_page) && ! is_single() && ! is_page() ) {
                return esc_html__('Blog', 'Shop Jo');
            }
        ...
        // Search Page
        if ( is_search() ) {
            return sprintf( esc_html__( 'Search Results for: %s', 'Shop Jo' ), get_search_query() );
        }

        // Archive Pages
        if ( is_archive() ) {
            if ( is_author() ) {
                return sprintf( esc_html__( 'All posts by %s', 'Shop Jo' ), get_the_author() );
            }
            elseif ( is_day() ) {
                return sprintf( esc_html__( 'Daily Archives: %s', 'Shop Jo' ), get_the_date() );
            }
            elseif ( is_month() ) {
                return sprintf( esc_html__( 'Monthly Archives: %s', 'Shop Jo'), get_the_date( _x( 'F Y', 'monthly archives date format', 'xstore' ) ) );
            }
            elseif ( is_year() ) {
                return sprintf( esc_html__( 'Yearly Archives: %s', 'Shop Jo' ), get_the_date( _x( 'Y', 'yearly archives date format', 'xstore' ) ) );
            }
            elseif ( is_tag() ) {
                return sprintf( esc_html__( 'Tag Archives: %s', 'Shop Jo' ), single_tag_title( '', false ) );
            }
            elseif ( is_category() ) {
                return sprintf( esc_html__( 'Category Archives: %s', 'Shop Jo' ), single_cat_title( '', false ) );
            }
            elseif ( is_tax( 'post_format', 'post-format-aside' ) ) {
                return esc_html__( 'Asides', 'Shop Jo' );
            }
            elseif ( is_tax( 'post_format', 'post-format-video' ) ) {
                return esc_html__( 'Videos', 'Shop Jo' );
            }
            elseif ( is_tax( 'post_format', 'post-format-audio' ) ) {
                return esc_html__( 'Audio', 'Shop Jo' );
            }
            elseif ( is_tax( 'post_format', 'post-format-quote' ) ) {
                return esc_html__( 'Quotes', 'Shop Jo' );
            }
            elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) {
                return esc_html__( 'Galleries', 'Shop Jo' );
            }
            elseif ( is_tax( 'portfolio_category' ) ) {
                return single_term_title();
            }
            elseif( function_exists('is_bbpress') && is_bbpress() ){
                return esc_html__('Forums', 'Shop Jo');
            }
            else {
                return esc_html__( 'Archives', 'Shop Jo' );
            }
        }
            ...
            if ( is_user_logged_in() ) {
                if( class_exists('WooCommerce')) {
                    $links['my-account'] = array(
                        ...
                        'title' => esc_html__( 'My Account', 'Shop Jo' ),
                        'submenu' => '<ul>'
                    );
                ...
                $login_text = ($short) ? esc_html__( 'Sign In', 'Shop Jo' ): esc_html__( 'Sign In or Create an account', 'Shop Jo' );
                ...
                if(!empty($reg_id)) {
                    $links['register'] = array(
                        ...
                        'title' => esc_html__( 'Register', 'Shop Jo' )
                    );
                }
